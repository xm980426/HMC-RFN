...
% Set spurious pulse ratio and PRI jitter rate as environmental variables, pulse loss rates (0.2, 0.4) (exp2_1) -
% Process in parallel between different Monte Carlo experiments (exp2_1_2)
close all; clear;
...
IterNum = 50;                                                              % Number of Monte Carlo trials under current conditions
DeleavingResult = [];
Time = zeros(Vnum, IterNum);                                               % Storage for sorting time
T = 1e4;                                                                   % Total observation duration
Pm = 0.2;                                                                  % Pulse loss rate - target source parameter
PRItype_set = [{'FixedPri'}, 'JaggingPri', {'GroupPri'}, {'SlipPri'}, {'SinedPri'}];
PRItype = 2;                                                               % PRI modulation information - target source parameter
...
%%
Mat = Indic;
save(['doc_paper3_ours_exp2_1_2_', cell2mat(PRItype_set(PRItype)), '-', num2str(Sp_setting), '-', num2str(Pm_setting), '-', num2str(Alpha), '-', num2str(Pm), '.mat'], 'Mat');
